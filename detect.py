...
model = YOLO(pretrained_model_path)  # pretrained YOLO11n model
...
for result in results:
    ...
    stem = Path(images[i]).stem.split('.')[0]
    result.save(filename=output_path+stem+"_yolo11n.pt.jpg")  # save to disk
    i+=1
